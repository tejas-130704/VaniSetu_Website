...
export default function DetailModal({ isOpen, onClose, sectionType, title }: DetailModalProps) {
    const content = mockResponses[sectionType] || mockResponses.problem || "No details available."

    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-3xl max-h-[80vh] bg-black/90 border border-white/20 text-white backdrop-blur-xl">
                <DialogHeader>
                    <DialogTitle className="text-2xl font-light text-cyan-400 mb-4">{title}</DialogTitle>
                </DialogHeader>
                <ScrollArea className="h-[60vh] pr-4">
                    <div className="whitespace-pre-wrap text-white/80 font-light leading-relaxed text-base space-y-4">
                        {content}
                    </div>
                </ScrollArea>
            </DialogContent>
